...
class DBInputDialog(QDialog):
    ...
    def ask_db_name(parent=None, default_location="", default_name=""):
        ...
        """
        dialog = DBInputDialog(parent, default_location, default_name)
        result = dialog.exec()
...
class DatabasePathDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.name_input.textChanged.connect(self.update_full_path)
        ...
        fullpath_label = QLabel("Full Path:")
        self.fullpath_output = QLineEdit()
        self.fullpath_output.setReadOnly(True)
        fullpath_layout.addWidget(fullpath_label)
        fullpath_layout.addWidget(self.fullpath_output)
        layout.addLayout(fullpath_layout)
    ...
    def update_full_path(self):
        ...
        if name and location:
            ...
            full_path = ""
        self.fullpath_output.setText(full_path)

    def get_full_path(self):
        return self.fullpath_output.text()

    @classmethod
    def ask_db_name(cls, parent=None):
        dialog = cls(parent)
        result = dialog.exec()
        if result == QDialog.DialogCode.Accepted:
            return dialog.get_full_path()
        return None
